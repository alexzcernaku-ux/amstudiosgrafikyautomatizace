        ...
        "properties": {
            "slides": {
                ...
                "items": {
                    ...
                    "required": ["type"],
                    # Bez týhle podmíněné validace šlo poslat slide s jen "type" a
                    # ničím jiným — vyrenderoval by se prázdný slide (jen textura,
                    # číslo stránky a swipe, nic navíc). Teď každý typ vynucuje svá
                    # skutečně nutná pole, ať se tohle nemůže stát.
                    "allOf": [
                        {"if": {"properties": {"type": {"const": "stat_cover"}}},
                         "then": {"required": ["stat", "stat_popis"]}},
                        {"if": {"properties": {"type": {"const": "quote_cover"}}},
                         "then": {"required": ["quote"]}},
                        {"if": {"properties": {"type": {"const": "text_highlight"}}},
                         "then": {"required": ["nadpis", "text"]}},
                        {"if": {"properties": {"type": {"const": "list_x"}}},
                         "then": {"required": ["nadpis", "body"]}},
                        {"if": {"properties": {"type": {"const": "titled_list"}}},
                         "then": {"required": ["nadpis", "body"]}},
                        {"if": {"properties": {"type": {"const": "checklist"}}},
                         "then": {"required": ["nadpis", "body"]}},
                        {"if": {"properties": {"type": {"const": "comparison"}}},
                         "then": {"required": ["nadpis", "body_spatne", "body_dobre"]}},
