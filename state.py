...
def update_request(thread_ts: str, **fields):
    # "now()" jako obyčejný text by Postgres nepřijal jako platné datum (na rozdíl
    # od SQL, tady to jde přes REST/JSON, ne přes přímý SQL příkaz) — proto se
    # spočítá skutečný časový údaj v Pythonu.
